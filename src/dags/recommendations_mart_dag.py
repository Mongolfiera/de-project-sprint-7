...
def load_events(events_path: str, start_date: str, end_date: str, spark) -> DataFrame:
    date_d = datetime.strptime(end_date, '%Y-%m-%d')
    ...
    events_df = (
        ...
        .withColumn('processed_dttm', F.lit(date_d))
    ...
    ).selectExpr('event_type', 'date', 'event_dt', 'processed_dttm', 'user_id',
                 'message_id', 'message_to', 'event_phi', 'event_lambda',
                 'subscription_channel', 'subscription_user')
...
def get_subscriptions_pairs(events_df: DataFrame) -> DataFrame:
    subscribtions = (
        ...
        .select('user_id', 'event_phi', 'event_lambda', 'subscription_channel', 'processed_dttm').distinct()
    )
...
def get_unique_subscriptions_pairs(subscribtions_pairs: DataFrame, messages_pairs: DataFrame) -> DataFrame:
    ...
    unique_subscribtions_pairs = (
        ...
        .select('user_id_left', 'user_id_right', 'event_phi', 'event_lambda', 'processed_dttm').distinct())
...
def add_event_timezone(events_with_city_df: DataFrame, geo_data_df: DataFrame) -> DataFrame:
    ...
    events_with_timezone_df = (
        ...
        .selectExpr('user_id_left', 'user_id_right', 'processed_dttm', 'city_id as zone_id', 'timezone as local_time').distinct()
    )
...
def main():
    ...
    conf = SparkConf().setAppName('RecommendationsMartJob')
    ...
    # получаем пары пользователей, подписанных на одинаковые каналы
